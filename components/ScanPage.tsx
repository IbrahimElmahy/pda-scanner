...
type SessionState = 'ready' | 'scanning' | 'summary' | 'saving';
...
const ScanPage: React.FC<{ navigate: (page: Page) => void }> = ({ navigate }) => {
  ...
  const handleFinishSession = async () => {
    ...
    try {
        ...
        playBeep(false);
        setSessionState('scanning'); // Go back to scanning on error
    }
  };
    ...
    <div className="space-y-6">
      <div className="bg-white p-4 rounded-lg shadow-md">
        <h2 className="text-xl font-bold text-gray-800 mb-4">
            {sessionState === 'scanning' ? `جلسة مسح لـ: ${currentCompany?.name}` : 'بدء جلسة مسح'}
        </h2>
        <div className="space-y-4">
          ...
          {sessionState === 'scanning' && (
             <button onClick={handleFinishSession} className="w-full px-6 py-4 border border-transparent rounded-md shadow-sm text-lg font-medium text-white bg-green-600 hover:bg-green-700 disabled:bg-gray-400" disabled={sessionScans.length === 0}>
                إنهاء وحفظ الجلسة
             </button>
          )}
           {sessionState === 'saving' && (
      ...
      {sessionState === 'scanning' && (
        <div className="bg-white p-4 rounded-lg shadow-md">
          <h3 className="text-xl font-bold text-gray-800 mb-4">شحنات الجلسة الحالية ({totalCartonsInSession})</h3>
          <div className="space-y-3 max-h-[45vh] overflow-y-auto no-scrollbar">
            {sessionScans.length === 0 ? (
              <p className="text-gray-500 text-center py-4">في انتظار أول عملية مسح...</p>
            ) : (
